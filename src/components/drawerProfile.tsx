...
const DrawerProfile = () => {
  ...
  return (
    <div
      className={`z-10 mt-[4rem] pb-16 h-full fixed flex flex-col  transition-all ${
        !open
    ...
    >
      {userProfile.email ? (
        <button
          onClick={toggleOpen}
          className="absolute z-[1000] top-3 right-0 w-[1.7rem] h-[1.7rem] flex items-center justify-center transition-all"
        >
          {open ? (
        ...
        <div className="absolute z-20 top-[4rem] left-[.5rem] w-[1.5rem] h-[1.5rem] animate-spin border-b-2 border white rounded-full"></div>
      )}
      <div
